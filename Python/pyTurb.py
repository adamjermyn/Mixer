...
_turb = ctypes.CDLL(d)
_turb.coeffs2.argtypes = [ctypes.c_double for _ in range(9)] + 2 * [ctypes.c_int]
_turb.coeffs2.restype = ctypes.POINTER(ctypes.c_double)
_turb.coeffs3.argtypes = [ctypes.c_double for _ in range(12)] + 2 * [ctypes.c_int]
_turb.coeffs3.restype = ctypes.POINTER(ctypes.c_double)

_turb.coeffs2spherical.argtypes = [ctypes.c_double for _ in range(10)] + 2 * [ctypes.c_int]
_turb.coeffs2spherical.restype = ctypes.POINTER(ctypes.c_double)
_turb.coeffs3spherical.argtypes = [ctypes.c_double for _ in range(13)] + 2 * [ctypes.c_int]
_turb.coeffs3spherical.restype = ctypes.POINTER(ctypes.c_double)

array_type = ctypes.c_int * 36
_turb.coeffs2sphericalSpecific.argtypes = [ctypes.POINTER(ctypes.c_int)] + [ctypes.c_double for _ in range(10)] + 2 * [ctypes.c_int]
_turb.coeffs2sphericalSpecific.restype = ctypes.POINTER(ctypes.c_double)
_turb.coeffs3sphericalSpecific.argtypes = [ctypes.POINTER(ctypes.c_int)] + [ctypes.c_double for _ in range(13)] + 2 * [ctypes.c_int]
...
_turb.coeffs2sphericalSpecificBox.argtypes = [ctypes.POINTER(ctypes.c_double) for _ in range(2)] + [ctypes.POINTER(ctypes.c_int)] + [ctypes.c_double for _ in range(9)] + [ctypes.c_int, ctypes.c_double, ctypes.c_int]
_turb.coeffs2sphericalSpecificBox.restype = ctypes.POINTER(ctypes.c_double)
_turb.coeffs3sphericalSpecificBox.argtypes = [ctypes.POINTER(ctypes.c_double) for _ in range(2)] + [ctypes.POINTER(ctypes.c_int)] + [ctypes.c_double for _ in range(12)] + [ctypes.c_int, ctypes.c_double, ctypes.c_int]
...
def correlator(k, kT, kP, B, tB, pB, omega, w, tW, tS, tP, N2, eps=1e-20, order=1):
	global _turb
	...
	res = _turb.correlator(ctypes.c_double(k),ctypes.c_double(kT),ctypes.c_double(kP),\
			...
			ctypes.c_double(tS),ctypes.c_double(tP),ctypes.c_double(N2), ctypes.c_double(eps), ctypes.c_int(order))
...
def coeffs2(omega, w, tW, tS, tP, N2, tolr, tola, maxEval, eps=1e-10, order=1):
	global _turb

	res = _turb.coeffs2(ctypes.c_double(tolr),ctypes.c_double(tola),\
			ctypes.c_double(omega),ctypes.c_double(w),ctypes.c_double(tW),\
			ctypes.c_double(tS),ctypes.c_double(tP),ctypes.c_double(N2),ctypes.c_int(maxEval), ctypes.c_double(eps), ctypes.c_int(order))
...
def coeffs3(B, tB, pB, omega, w, tW, tS, tP, N2, tolr, tola, maxEval, eps=1e-10, order=1):
	global _turb
	...
	res = _turb.coeffs3(ctypes.c_double(tolr),ctypes.c_double(tola),\
			...
			ctypes.c_int(maxEval), ctypes.c_double(eps), ctypes.c_int(order))
...
def coeffs2spherical(theta, omega, w, tW, tS, tP, N2, tolr, tola, maxEval, eps=1e-10, order=1):
	global _turb
...
def coeffs3spherical(theta, B, tB, pB, omega, w, tW, tS, tP, N2, tolr, tola, maxEval, eps=1e-10, order=1):
	global _turb
	...
	res = _turb.coeffs3spherical(ctypes.c_double(theta), ctypes.c_double(tolr),\
			...
			ctypes.c_double(N2),ctypes.c_int(maxEval), ctypes.c_double(eps), ctypes.c_int(order))
...
def coeffs2sphericalSpecific(output, theta, omega, w, tW, tS, tP, N2, tolr, tola, maxEval, eps=1e-10, order=1):
	global _turb
...
def coeffs3sphericalSpecific(output, theta, B, tB, pB, omega, w, tW, tS, tP, N2, tolr, tola, maxEval, eps=1e-10, order=1):
	global _turb
...
def coeffs2sphericalSpecificBox(mins, maxs, output, theta, omega, w, tW, tS, tP, N2, tolr, tola, maxEval, eps=1e-10, order=1):
	global _turb
...
def coeffs3sphericalSpecificBox(mins, maxs, output, theta, B, tB, pB, omega, w, tW, tS, tP, N2, tolr, tola, maxEval, eps=1e-10, order=1):
	global _turb
